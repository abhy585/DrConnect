...
export default function TestUpload() {
  ...
  const [ocrResponse, setOCRResponse] = React.useState({
    //response of OCR Service (value contains the id)
    resultItems: [
      {
        files: [
          {
  ...
  const [uploadedFile, setUploadedFile] = React.useState({
    //response of file uploaded to Content Storage
    id: "",
    mimeType: "",
    size: "",
  });
  ...
  const [uploadedFileIdPlaceholder, setUploadedFileIdPlaceholder] =
    React.useState("");
  ...
  /**
   * Step 3 - createSearchablePDF() - Call OCR Service ############################################################################Hackathon
   */
  ...
  /**
   * Step 4B - uploadFileToCSS() - upload File to Content Storage ############################################################################Hackathon
   */
  ...
  /**
   * Step 5B - createFolder() - Create new folder using CMS              ############################################################################Hackathon by doctor of patient name
   */
  ...
  /**
   * Step 6 - createMetadataForFile() - Create metadata for PDF file        ############################################################################Hackathon
   */
  async function createMetadataForFile() {
    ...
    const pdfFileName =
      ocrResponse.resultItems[0].files[0].name +
      "." +
      ocrResponse.resultItems[0].files[0].fileType;
    ...
    if (data.total > 0) {
      console.log(
        "Metadata  already exists. # of existing versions =",
        data.total
      );

      //Add file as new version
      const url2 = `${properties.base_url}/cms/instances/file/cms_file/${
        data._embedded.collection[data.total - 1].id
      }/nextVersion`;
      const fetchOptions2 = {
        method: "POST",
        headers: {
  ...
  return (
    <main>
      <h1 className="ot2-sample-header">
        Create and Store a Text Searchable PDF using OpenText CMS, CSS and
        Capture Service
      </h1>
      <br />
      <div className="ot2-body">
        <div>
          ...
          />
          <br />
          <button className="button-token" onClick={getAuthToken}>
            Get Token
          </button>
          <br />
          <br />
        ...
        <div>
          ...
          <input
            ...
            name="capturedFileId"
            value={capturedFileId}
            placeholder={capFileIdPlaceholder}
            size="35"
            readOnly
          />
          <br />
        ...
        <div>
          ...
          <button onClick={createSearchablePDF}>
          ...
          <input
            ...
            placeholder={ocrRespIdPlaceholder}
            size="35"
            readOnly
          />
          <br />
        ...
        <div>
          ...
          &nbsp;
          <button onClick={retrieveFileFromCaptureService}>
            Retrieve file from Capture Service
          </button>
          &nbsp;&nbsp;<i>{retrieveStatus}</i>
          <br />
          <br />
          ...
          &nbsp;
          <button onClick={uploadFileToCSS}>
          ...
          <input
            ...
            placeholder={uploadedFileIdPlaceholder}
            size="35"
            readOnly
          />
          <br />
        ...
        <div>
          ...
          <label htmlFor="folderName">
            <b>Step 1: </b>&nbsp;Provide a folder name:{" "}
          </label>
          &nbsp;
          <input
            type="text"
        ...
        <div>
          ...
          <button onClick={retrieveFile}>Download the PDF File</button>
          <br />
          <br />
        </div>
        <hr align="left" />
